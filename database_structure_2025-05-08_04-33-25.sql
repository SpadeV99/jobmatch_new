...
CREATE TABLE `job_applications` (
  ...
  `status` enum('pending','shortlisted','interview_scheduled','interviewed','offered','rejected','withdrawn','hired') COLLATE utf8mb4_general_ci DEFAULT 'pending',
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  KEY `job_id` (`job_id`),
...
-- id (int), user_id (int), job_id (int), apply_date (datetime), resume_path (varchar(255)), cover_letter (text), status (enum('pending','shortlisted','interview_scheduled','interviewed','offered','rejected','withdrawn','hired'))
